...
CONTROLLA_OGNI_MINUTI = 5   # frequenza controllo mercati
...
def cmd_prezzi() -> str:
    """Prezzi aggiornati di tutte le crypto."""
    righe = ["💰 <b>PREZZI AGGIORNATI</b>\n"]
    for crypto_id, name in CRYPTOS.items():
        prezzi, _ = ottieni_dati(crypto_id, giorni=1)
        if prezzi:
            righe.append(f"  <b>{name}</b>  {fmt_prezzo(prezzi[-1])}")
        else:
...
def main() -> None:
    ...
    while True:
        adesso = datetime.now()
        print(f"\n[{adesso.strftime('%H:%M:%S')}] ── Controllo ──")

        try:
            gestisci_comandi(stato)       # risponde ai tuoi comandi Telegram
            controlla_ingressi(stato)     # cerca segnali di acquisto
            controlla_uscite(stato)       # monitora TP/SL delle posizioni aperte
        except Exception as e:
            print(f"  [ERRORE] {e}")
