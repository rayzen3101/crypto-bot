...
def msg_compra(name, prezzo, rsi, vol_att, vol_media) -> str:
    vp  = (vol_att / vol_media - 1) * 100 if vol_media else 0
    ...
    return (
        ...
        f"  Volume   : +{vp:.1f}% ✅  (media {fmt_vol(vol_media)})\n\n"
...
def controlla_ingressi(stato: dict) -> None:
    for crypto_id, name in CRYPTOS.items():
        ...
        if rsi_ok and macd_ok and v_ok:
            print(f"  🟢 SEGNALE COMPRA su {name}!")
            invia_messaggio(msg_compra(name, prezzo, rsi, v_att, v_med))
